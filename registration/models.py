...
class RegistrationManager(models.Manager):
    ...
    def add_groups(self, user, groups):
        for group in groups:
            print group
            selected_group = Group.objects.filter(name = group)
    ...
    def create_inactive_user(self, username, email, send_email=True, groups=None):
        """
        ...
        new_user.save()
        self.add_groups(new_user, groups)
        ...
        if send_email:
            ...
            subject = "[AGESUR] Activez votre nouveau compte"
            ...
            message_context = Context({ 'site_url': 'http://%s/' % current_domain,
                                        'activation_key': new_user.password,
